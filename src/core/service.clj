...
(defn find-user
  ...
  (let [user-id (get-in request [:path-params :user-id])
        user (user-repository/find-user user-id)]
    (if user
...
(defn create-user
  ...
  (let [user (:body request)]
    (user-repository/create-user user)
...
(def routes #{["/" :get (conj common-interceptors home-page) :route-name :home]
              ["/users/:user-id/name" :get (conj common-interceptors user-name) :route-name :user-name]
              ["/users/:user-id" :get (conj common-interceptors find-user) :route-name :get-user]
              ["/users/:user-id" :post (conj common-interceptors user) :route-name :create-user]
